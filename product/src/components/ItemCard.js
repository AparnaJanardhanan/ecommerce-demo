...
const ItemCard = ({ selectedComponent, closeSelected, searchQuery, minPrice, maxPrice }) => {
    ...
    const handleSearch = () => {
        ...
        setSearchFilterData(filteredProducts);
    };
    ...
    console.log("products", searchFilterData && searchFilterData, filteredData && filteredData, selectedComponent);
    ...
    return (
        <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 rounded-full px-32 pb-4">
                {searchFilterData ? (
                    searchFilterData && searchFilterData.length !== 0 ? (searchFilterData.map((item) => (
                        <div className="w-60 h-min p-2 bg-white rounded-lg shadow-2xl" key={item.id}>
